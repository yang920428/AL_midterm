...
def Q_learning(a, W, robot, goal, laser, R, terminal):
    ...
    f_current = features(robot, goal, laser, a+1)  # +1 to match MATLAB's 1-based action indexing
    ...
    if terminal:
        ...
        for a_next in range(num_actions):
            f_next = features(robot, goal, laser, a_next+1)  # +1 for MATLAB compatibility
    ...
    if random.random() < epsilon:
        ...
        for a_candidate in range(num_actions):
            f = features(robot, goal, laser, a_candidate+1)  # +1 for MATLAB compatibility
